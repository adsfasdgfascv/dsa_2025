...
void initialize_LL(LinkedSET * set){
    set = NULL;
}

void insert_LL(LinkedSET * set, int element){
    LinkedSet * trav = set;
    
    while(trav != NULL){
        if(trav->data == element) return;
        trav = trav->next;
    }
    
    LinkedSET * newNode = malloc(sizeof(dtype));
    newNode->next = set;
    newNode->data = element;
    set = newNode;
}

void delete_LL(LinkedSET * set, int element){
    LinkedSET 
}

bool find_LL(SET_arr set, int element) {

SET_arr Sunion_LL(SET_arr A, SET_arr B) {

SET_arr intersection_LL(SET_arr A, SET_arr B) {

SET_arr difference_LL(SET_arr A, SET_arr B) {

void display_LL(SET_arr A) {
