...
void MinHeapifyNDX(Heap *h, int index){
    ...
    if(left < h->count && h->elem[left] > h->elem[large]) large = left; //check existence of left node
    if(right < h->count && h->elem[right] > h->elem[large]) large = right; //check existence of right node and check if it min to arr[ndx]
    
    //swap it
    if(large != index){
        int temp = h->elem[index];
        h->elem[index] = h->elem[large];
        h->elem[large] = temp;
        MinHeapifyNDX(h, large); //recursion
    }
}
...
int DeleteMin(Heap *h) {
    ...
    int min = h->elem[h->count-1];
    h->elem[h->count-1] = h->elem[0];
    h->count--;
